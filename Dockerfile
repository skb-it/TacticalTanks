...
WORKDIR /src
COPY ["TacticalTanks.Api.csproj", "."]
RUN dotnet restore "./TacticalTanks.Api.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./TacticalTanks.Api.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./TacticalTanks.Api.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "TacticalTanks.Api.dll"]